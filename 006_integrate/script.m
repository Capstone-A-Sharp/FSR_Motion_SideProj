...
figure;
axis equal
axis(0.6*[-height height -height height -height height])
view(3)
colormap('parula');
colorbar;
caxis([0 100])
...
% 
    ...
    for i = 1:numSides
        ...
        for j = 1:(numCols - 1)
            z1 = (j - 1) / (numCols - 1) * height - height/2;
            z2 = j / (numCols - 1) * height - height/2;
            ...
            verts = ([1 0 0;
                    0 cos(pi/2) -sin(pi/2);
                    0 sin(pi/2)  cos(pi/2)] * verts')';

            verts = ([1 0 0;
                    0 cos(roll) -sin(roll);
                    0 sin(roll)  cos(roll)] * verts')';

            verts = ([cos(pitch) 0 -sin(pitch); ...
                    0 1 0; ...
                    sin(pitch) 0 cos(pitch)] * verts')';
    ...
    hold on;
    plot3([0, radius * 1.5], [0, 0], [0, 0], 'r-', 'LineWidth', 2);  % 빨간 축
    hold off;
    drawnow;
end
